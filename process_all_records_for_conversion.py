...
def main(source_type: str = None, generation_mode: str = None, theme: str = None, subfolder: str = None, parent_id: int = None):
    ...
    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'batch_convert_to_mp3_v2.py')
    ...
    while True:
        ...
        cursor = conn.cursor()
        if parent_id:
            cursor.execute("SELECT COUNT(*) FROM podcast_download WHERE parent_configuration_id = ? AND download_state = 1 AND conversion_state = 0", (parent_id,))
        else:
            cursor.execute("""SELECT COUNT(*) FROM podcast_download pd JOIN parent_configuration pc ON pd.parent_configuration_id = pc.id
                WHERE pc.source_type = ? AND pc.generation_mode = ? AND pc.podcast_theme = ? AND pc.podcast_subtheme = ? AND pd.download_state = 1 AND pd.conversion_state = 0""",
