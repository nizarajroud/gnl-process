#!/usr/bin/env python3
"""Process all matching records for download by calling the main script once per record."""
...
def setup_logging(parent_id):
    ...
    log_file = os.path.join(logs_dir, f"download_{datetime.now().strftime('%Y%m%d_%H%M%S')}_parent{parent_id or 'all'}.log")
    logger = logging.getLogger('gnl_download')
...
def main(source_type: str = None, generation_mode: str = None, theme: str = None, subfolder: str = None, parent_id: int = None):
    ...
    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'nllm-aws-asl-download-rename-gnl_v2.py')
    ...
    query += " ORDER BY pd.parent_configuration_id ASC, CAST(REPLACE(REPLACE(REPLACE(pd.source_id, 'p', ''), 'q', ''), '.pdf', '') AS INTEGER) ASC"
    cursor.execute(query, params)
    records = cursor.fetchall()
    conn.close()

    if not records:
        log_print("\n✓ No records to download.", logger)
        sys.exit(0)
    ...
    log_print(f"\nStarting download for {total} records (parent_id={parent_id})", logger)

    for i, (record_id, source_id) in enumerate(records, 1):
        log_print(f"\n{'='*60}", logger)
        log_print(f"[{i}/{total}] Downloading record {record_id} ({source_id})", logger)
        log_print(f"{'='*60}\n", logger)
        ...
        if TRACING and logger:
            ...
            with open(log_file_path, 'a') as log_f:
                try:
                    result = subprocess.run(cmd, stdout=log_f, stderr=log_f, text=True, timeout=DOWNLOAD_TIMEOUT)
                except subprocess.TimeoutExpired:
                    log_print(f"⏱ Record {record_id} ({source_id}) timed out after {DOWNLOAD_TIMEOUT}s.", logger)
        ...
        # Cleanup Chrome between records
        ...
        if result.returncode == 0:
            ...
            log_print(f"✓ Record {record_id} ({source_id}) downloaded.", logger)
        else:
