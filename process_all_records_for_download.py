...
def main(source_type: str = None, generation_mode: str = None, theme: str = None, subfolder: str = None, parent_id: int = None):
    ...
    while True:
        ...
        cursor = conn.cursor()
        if parent_id:
            cursor.execute("SELECT COUNT(*) FROM podcast_download WHERE parent_configuration_id = ? AND generation_state = 1 AND download_state = 0", (parent_id,))
        else:
            cursor.execute("""SELECT COUNT(*) FROM podcast_download pd JOIN parent_configuration pc ON pd.parent_configuration_id = pc.id
                WHERE pc.source_type = ? AND pc.generation_mode = ? AND pc.podcast_theme = ? AND pc.podcast_subtheme = ? AND pd.generation_state = 1 AND pd.download_state = 0""",
