...
if [ "$GENERATION_MODE" = "single" ]; then
    FILE_PATH=$(find "$LOCAL_STORAGE_PATH" -name "$1" -type f 2>/dev/null)
    if [ -n "$FILE_PATH" ]; then
    ...
    FILE_PATH=$(find "$LOCAL_STORAGE_PATH" -name "$1" -type f 2>/dev/null | head -1)
    if [ -n "$FILE_PATH" ]; then
        PARENT_DIR=$(dirname "$FILE_PATH")
        PARENT_NAME=$(basename "$PARENT_DIR")
        FILES_JSON=$(find "$PARENT_DIR" -type f 2>/dev/null | while read file; do
